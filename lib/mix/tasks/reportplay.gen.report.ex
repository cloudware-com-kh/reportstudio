defmodule Mix.Tasks.Reportplay.Gen.Report.Docs do
  @moduledoc false
  ...
  def example do
    "mix reportplay.gen.report student"
  end
...
defmodule Mix.Tasks.Reportplay.Gen.Report do
  ...
  def igniter(igniter) do
    ...
    controller_path = "lib/reportplay_web/controllers/#{name}_report_controller.ex"
    html_path = "lib/reportplay_web/controllers/#{name}_report_html.ex"
    heex_path = "lib/reportplay_web/controllers/#{name}_report_html/report.html.heex"
    css_path = "assets/css/#{name}_report.css"

    controller_content = """
    defmodule ReportplayWeb.Controllers.#{module_name_suffix}ReportController do
    ...
    html_content = """
    defmodule ReportplayWeb.Controllers.#{module_name_suffix}ReportHTML do
    ...
    @source "../../lib/reportplay_web/controllers/#{name}_report_html/report.html.heex";
    """
    ...
    # Append route to router.ex using manual update to target the ReportplayWeb scope
    |> Igniter.update_file("lib/reportplay_web/router.ex", fn source ->
    ...
    |> Igniter.Project.Config.configure(
      "dev.exs",
      :reportplay,
      [ReportplayWeb.Endpoint, :watchers, watcher_key],
      {:code,
    ...
    |> Igniter.update_file("mix.exs", fn source ->
      ...
      new_content =
        if String.contains?(content, "tailwind #{name}_report") do
          ...
          |> String.replace(
            "\"tailwind reportplay\",",
            "\"tailwind reportplay\", \"tailwind #{name}_report\","
          )
          |> String.replace(
